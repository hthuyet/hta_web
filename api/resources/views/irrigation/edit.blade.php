...
<div class="box box-primary">
    <div class="box-header with-border">
        <h3 class="box-title">Sửa irrigation</h3>
    </div><!-- /.box-header -->
    
    <!-- form start -->
    {!! Form::open(array("id" => "formIrri",'onsubmit' => 'return validateOnSubmit(this);', 'route' => ['irrigation.update', $irrigation->id], 'method' => 'PUT')) !!}
    <div class="box-body">
            ...
            <div class="col-sm-6">
                <div class="row">
                    <div class="form-group">
                        ...
                        <div class="col-sm-8">
                            {!! Form::text('area_name', $irrigation->area_name, ["id"=>"txtName", 'class' => 'form-control', 'placeholder' => 'Tên khu vực']) !!}
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="form-group">
                        ...
                        <div class="col-sm-8">
                            {!! Form::text('code', $irrigation->code, ["id"=>"txtCode",'class' => 'form-control', 'placeholder' => 'Mã quy trình']) !!}
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="form-group">
                        ...
                        <div class="col-sm-8">
                            {!! Form::text('from_date', date('d/m/Y', strtotime($irrigation->from_date)), ["id"=>"txtForm",'class' => 'form-control datepicker', 'placeholder' => 'Ngày bắt đầu']) !!}
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="form-group">
                        ...
                        <div class="col-sm-8">
                            {!! Form::text('to_date', date('d/m/Y', strtotime($irrigation->to_date)), ["id"=>"txtTo",'class' => 'form-control datepicker', 'placeholder' => 'Ngày kết thúc']) !!}
                        </div>
                    </div>
                </div>
            </div>
                            ...
                            <table id="irrigationDetailTable" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example1_info">
                                ...
                                <tr role="row" class="odd">
                                    <td class="form-group trRow">
                                        {!! Form::select('items['.$loop.'][device_id]', $devices, $aDetail->device_id, ['class' => 'form-control select2','id' => 'device_id_' . $loop])  !!}
                                        <input type="hidden" name="items[{{$loop}}][id]" value="{{$aDetail->id}}" />
                                    </td>
                                    <td class="form-group trRow">{!! Form::text('items['.$loop.'][step]', $aDetail->step, ['class' => 'form-control', 'placeholder' => 'Bước']) !!}</td>
                                    <td class="form-group trRow">{!! Form::text('items['.$loop.'][port]', $aDetail->port, ["id" => 'port_' . $loop,'class' => 'form-control', 'placeholder' => 'Cổng']) !!}</td>
                                    <td class="form-group trRow">{!! Form::text('items['.$loop.'][condition]', $aDetail->condition, ['class' => 'form-control', 'placeholder' => 'Điều kiện']) !!}</td>
                                    <td class="form-group trRow">{!! Form::text('items['.$loop.'][weight]', $aDetail->weight, ['class' => 'form-control', 'placeholder' => 'Khối lượng']) !!}</td>
                                    <td class="form-group trRow">{!! Form::text('items['.$loop.'][from]', $aDetail->from, ["id" => "from_".$loop,'class' => 'form-control timemask', 'placeholder' => 'Enter key',"onkeyup"=>'updateEndTime("'.$loop.'")', "onchange"=>'updateEndTime("'.$loop.'")']) !!}</td>
                                    <td class="form-group trRow">{!! Form::text('items['.$loop.'][time]', $aDetail->time, ["id" => "time_".$loop,'class' => 'form-control', 'placeholder' => 'Enter Time', "onkeyup"=>'updateEndTime("'.$loop.'")', "onchange"=>'updateEndTime("'.$loop.'")']) !!}</td>
                                    <td class="form-group trRow">{!! Form::text('items['.$loop.'][to]', $aDetail->to, ["id" => "to_".$loop,'class' => 'form-control timemask', 'placeholder' => 'Kết thúc',"readOnly"=>true]) !!}</td>
                                    <td class="form-group trRow">
    ...
    <div class="box-footer">
        {!! Form::submit('Cập nhật',['class' => 'btn btn-primary',"onclick" => "onsubmitForm()"]) !!}
    </div>
    {!! Form::close() !!}
</div>
@endsection
...
 <script id="rowTemplate" type="text/x-jQuery-tmpl">
    ...
    <td class="form-group trRow">{!! Form::text('items[${stt}][step]', null, ['class' => 'form-control', 'placeholder' => 'Enter key']) !!}</td>
    <td class="form-group trRow">{!! Form::text('items[${stt}][port]', null, ["id" => 'port_${stt}','class' => 'form-control', 'placeholder' => 'Enter key']) !!}</td>
    <td class="form-group trRow">{!! Form::text('items[${stt}][condition]', null, ['class' => 'form-control', 'placeholder' => 'Enter key']) !!}</td>
    <td class="form-group trRow">{!! Form::text('items[${stt}][weight]', null, ['class' => 'form-control', 'placeholder' => 'Enter key']) !!}</td>
