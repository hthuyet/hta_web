...
angular.module('BlurAdmin.pages.device')
        .controller('DeviceCtrl', function ($scope, DeviceService, AclService, $uibModal, cfpLoadingBar,
            ...
            function updatePortHtml(device, portStatus) {
                try {
                    //[1,0,1,1]
                    var lstPort = JSON.parse(portStatus);
                    if (lstPort && lstPort.length > 0) {
                        var stt = 0;
                        lstPort.forEach(function (obj) {
                            if (obj == 1 || obj == "1") {
                                $("#port_" + device + "_" + stt).prop("checked", true);
                            } else {
                                $("#port_" + device + "_" + stt).prop("checked", false);
                            }
                            stt++;
                        });
                    }
                            ...
                            myWebsocket.open(listDevice, function (device) {
                                angular.forEach($scope.data, function (obj) {
                                    if (device.code == obj.code) {
                                        if (device.state != 1 && device.state != '1') {
                                            $("#" + obj.code).removeClass("offline");
                                            $("#" + obj.code).removeClass("online");
                                            $("#" + obj.code).addClass("offline");
                                            $("#state_" + obj.code).html("OFFLINE");
                                            obj.state = 0;
                                        } else {
                                            $("#" + obj.code).removeClass("offline");
                                            $("#" + obj.code).removeClass("online");
                                            $("#" + obj.code).addClass("online");
                                            $("#state_" + obj.code).html("ONLINE");
                                            obj.port_status = "" + device.portStatus;
                                            parseLstPort(obj);
                                            obj.state = 1;
                                            updatePortHtml(obj.code, device.portStatus);
//                                        $timeout(function () {
...
//        });
            ...
            $scope.add = function () {
                $scope.open({}, 'DeviceModalCtrl as ctrl', 'app/pages/device/view/edit.html', 'lg');
            };
...
//                });
            ...
            $scope.control2 = function (item) {
                ...
                item.switches = switches;
                $scope.open(item, 'ControlDeviceCtrl as ctrl', 'app/pages/device/view/control_1.html', 'lg', "");
            };
