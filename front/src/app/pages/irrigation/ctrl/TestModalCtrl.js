...
angular.module('BlurAdmin.pages.irrigation').controller('TestModalCtrl', [
    ...
    'toastrConfig', '$filter', 'editableOptions', 'editableThemes'
            , function ($scope, $uibModalInstance, $uibModal, IrrService, item, lstDevice, lstArea, toastr, toastrConfig,
                ...
                $scope.saveAdd = function (rowform, irrigation) {
//                    var fromDate = convertToDate(irrigation.from);
                    var timeForm = rowform.$data.from.split(":");
                    var timeIrr = rowform.$data.time;
                    if (timeForm && timeIrr) {
                        var hh = timeForm[0];
                        var mm = timeForm[1];
                        var ss = timeForm[2];
                        ...
                        console.log(date);
                        date.setSeconds(date.getSeconds() + parseInt(rowform.$data.time));
...
//                                showError(response.error);
                ...
                $scope.timemask = function () {
                    console.log("---timemask-------");
                    $(".timemask").inputmask({"mask": '99:99:99'});
                }
